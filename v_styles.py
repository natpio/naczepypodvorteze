...
def apply_supreme_ui():
    bg = load_vorteza_asset('bg_vorteza.png')
    st.markdown(f"""
        <style>
            ...
            /* PANELE DEEP GLASS - Naprawiają czytelność metryk */
            .v-tile-pro {{
                background: rgba(5, 5, 5, 0.96);
                padding: 2.8rem; border-radius: 4px; border: 1px solid rgba(181, 136, 99, 0.3);
                border-left: 10px solid var(--v-copper); box-shadow: 0 60px 150px #000; margin-bottom: 3.5rem; backdrop-filter: blur(45px);
            }}
