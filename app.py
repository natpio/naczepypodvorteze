...
def generate_color_palette(n):
    """Generuje n unikalnych i wyraźnych kolorów HEX."""
    colors = []
    random.seed(42) # Stałe ziarno, aby kolory produktów były powtarzalne w sesji
    for _ in range(n):
...
def load_products():
    try:
        with open('products.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
            # Zabezpieczenie przed brakującymi polami w json
            for p in data:
                if 'itemsPerCase' not in p: p['itemsPerCase'] = 1
                if 'weight' not in p: p['weight'] = 1
            return sorted(data, key=lambda x: x.get('name', ''))
    except:
        return [{"name": "Błąd ładowania products.json", "width": 80, "length": 120, "height": 140, "weight": 400, "canStack": True, "itemsPerCase": 1}]
...
# --- 5. WIZUALIZACJA 3D Z ETYKIETAMI ---
def draw_3d(placed_stacks, vehicle, color_map):
    ...
    for s in placed_stacks:
        for it in s['items']:
            ...
            # Nowoczesna etykieta hover
            hover_label = (f"<b>{it['name']}</b><br>"
                          f"Wymiary: {dx}x{dy}x{dz} cm<br>"
                          f"Waga: {it['weight']} kg<br>"
                          f"Poz. Z: {z0} cm")
...
if check_password():
    ...
    # Inicjalizacja mapy kolorów - każdy produkt dostaje swój unikalny kolor
    ...
    with st.sidebar:
        ...
        if st.button("Wyczyść plan", type="secondary", use_container_width=True):
            st.session_state.cargo = []
            st.rerun()

    if st.session_state.cargo:
        st.header("📋 Lista załadunkowa")
            ...
            for _, row in edited_df.iterrows():
                if row['actual_items'] > 0:
                    ...
                    for i in range(num_u):
                        ...
                        u['actual_items'] = r if (i == num_u-1 and r != 0) else ipc
                        new_cargo.append(u)
        ...
        st.header(f"📊 Plan: {len(fleet)} auto/a")
        for idx, truck in enumerate(fleet):
            with st.expander(f"🚛 Pojazd #{idx+1}", expanded=True):
                m1, m2, m3 = st.columns(3)
                m1.metric("LDM", f"{truck['ldm']:.2f}")
                m2.metric("Waga", f"{truck['weight']} kg")
                floor = sum(s['width']*s['length'] for s in truck['stacks'])
                m3.metric("Podłoga", f"{int(floor/(veh['L']*veh['W'])*100)}%")
