...
# --- 5. WYDAJNA WIZUALIZACJA 3D (ZMODYFIKOWANA: NACZEPA + SIATKA) ---
def draw_3d_pro(stacks, vehicle, color_map):
    fig = go.Figure()
    L, W, H = vehicle['L'], vehicle['W'], vehicle['H']

    # --- KONSTRUKCJA NACZEPY ---
    # Metaliczna tekstura i kolor dla ścian
    trailer_color = "#334155" # Ciemnoszary, metaliczny
    grid_color = "white"     # Kolor siatki
    grid_width = 1          # Grubość linii siatki

    # 1. Przednia ściana (od strony kabiny)
    fig.add_trace(go.Mesh3d(
        x=[0, 0, 0, 0], y=[0, W, W, 0], z=[0, 0, H, H],
        color=trailer_color, opacity=1, flatshading=True, hoverinfo='skip'
    ))

    # 2. Sufit
    fig.add_trace(go.Mesh3d(
        x=[0, L, L, 0], y=[0, 0, W, W], z=[H, H, H, H],
        color=trailer_color, opacity=1, flatshading=True, hoverinfo='skip'
    ))

    # 3. Prawa ściana boczna (daleka)
    fig.add_trace(go.Mesh3d(
    ...
    fig.add_trace(go.Mesh3d(
        ...
        color="#1e293b", opacity=1, flatshading=True, hoverinfo='skip'
    ))

    # --- SIATKA TECHNICZNA ---
    # Siatka na podłodze
    grid_spacing = 100 # Co 100cm
    # Linie wzdłużne
    for y in range(0, int(W) + 1, grid_spacing):
        fig.add_trace(go.Scatter3d(
            x=[0, L], y=[y, y], z=[1, 1], # 1cm powyżej podłogi
            mode='lines', line=dict(color=grid_color, width=grid_width), hoverinfo='skip'
        ))
    # Linie poprzeczne
    for x in range(0, int(L) + 1, grid_spacing):
        fig.add_trace(go.Scatter3d(
            x=[x, x], y=[0, W], z=[1, 1],
            mode='lines', line=dict(color=grid_color, width=grid_width), hoverinfo='skip'
        ))

    # Siatka na dalekiej ścianie bocznej (y=0)
    for z in range(0, int(H) + 1, grid_spacing):
        fig.add_trace(go.Scatter3d(
            x=[0, L], y=[1, 1], z=[z, z], # 1cm od ściany
            mode='lines', line=dict(color=grid_color, width=grid_width), hoverinfo='skip'
        ))
    for x in range(0, int(L) + 1, grid_spacing):
        fig.add_trace(go.Scatter3d(
    ...
    # Neonowy obrys zewnętrzny dla efektu PRO
    fig.add_trace(go.Scatter3d(
        x=[0, L, L, 0, 0, None, 0, 0, None, L, L, None, L, L, None, 0, 0],
        y=[0, 0, W, W, 0, None, 0, W, None, 0, W, None, 0, 0, None, 0, 0],
        z=[0, 0, 0, 0, 0, None, 0, 0, None, 0, 0, None, 0, H, None, 0, H],
    ...
    for s in stacks:
        for it in s['items']:
            ...
            fig.add_trace(go.Mesh3d(
                ...
                # customdata=[it['name'], dx, dy, dz, z]
            ))
            
            # Dodawanie krawędzi do wspólnego trace'a ( Single trace optimization)
            # Dół, góra i piony
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode='lines', 
        line=dict(color='black', width=2), # Czarne obramowanie skrzyń
        hoverinfo='skip'
    ))
    ...
    fig.update_layout(
        scene=dict(
            xaxis=dict(range=[0, L], title="", showgrid=False, showticklabels=False, backgroundcolor="rgba(0,0,0,0)"),
            yaxis=dict(range=[0, W], title="", showgrid=False, showticklabels=False, backgroundcolor="rgba(0,0,0,0)"),
            zaxis=dict(range=[0, H], title="", showgrid=False, showticklabels=False, backgroundcolor="rgba(0,0,0,0)"),
