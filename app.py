...
# --- 4. POPRAWIONY I ZOPTYMALIZOWANY SILNIK PAKOWANIA ---
            ...
            if item.get('canStack', True):
                for s in placed_stacks:
                    if s['canStackBase'] and (item['width'] == s['width'] and item['length'] == s['length']) and (s['currentH'] + item['height'] <= vehicle['H']):
                        it_c = item.copy(); it_c['z_pos'] = s['currentH']
                        s['items'].append(it_c); s['currentH'] += item['height']
                        curr_weight += item['weight']; added = True; break
            
            if added: continue

            # 2. Szukanie miejsca na podłodze (z rotacją)
            best_fit = None
            for w, l in [(item['width'], item['length']), (item['length'], item['width'])]:
                # Czy mieści się w obecnym rzędzie?
                if shelf_y + l <= vehicle['W'] and shelf_x + w <= vehicle['L']:
                    best_fit = (w, l, shelf_x, shelf_y); break
                # Czy mieści się w nowym rzędzie?
                elif shelf_x + shelf_max_x + w <= vehicle['L'] and l <= vehicle['W']:
                    best_fit = (w, l, shelf_x + shelf_max_x, 0); break

            if best_fit:
                fw, fl, px, py = best_fit
                if py == 0 and px > shelf_x: # Przejście do nowej linii
                    shelf_x, shelf_y, shelf_max_x = px, 0, 0
                
                it_c = item.copy(); it_c['z_pos'] = 0
                it_c['w_active'], it_c['l_active'] = fw, fl
                
                placed_stacks.append({
                    'x': px, 'y': py, 'width': fw, 'length': fl, 
                    'currentH': item['height'], 'canStackBase': item.get('canStack', True), 'items': [it_c]
                })
                shelf_y = py + fl
                shelf_max_x = max(shelf_max_x, fw)
                curr_weight += item['weight']
                max_reached_l = max(max_reached_l, px + fw)
                added = True
            else:
                not_placed.append(item)
        
        if not placed_stacks: break
        fleet.append({"stacks": placed_stacks, "weight": curr_weight, "ldm": max_reached_l/100})
        remaining_items = not_placed
                
    return fleet

# --- 5. WYDAJNA WIZUALIZACJA 3D ---
def draw_3d_pro(stacks, vehicle, color_map):
    ...
    for s in stacks:
        for it in s['items']:
            ...
            # Dodawanie krawędzi (Single trace optimization)
    ...
    fig.update_layout(
        scene=dict(
            xaxis=dict(range=[0, L], title="DL"), yaxis=dict(range=[0, W], title="SZ"), zaxis=dict(range=[0, H], title="WY"),
...
if check_password():
    ...
    with st.sidebar:
        ...
        if st.button("DODAJ", use_container_width=True) and sel_p:
    ...
    if st.session_state.cargo:
        ...
        sum_df = pd.DataFrame(st.session_state.cargo).groupby('name').size().reset_index(name='ilość')
        
        edited = st.data_editor(sum_df, hide_index=True, use_container_width=True, key="ed_v4")
        ...
        st.info("System gotowy. Dodaj sprzęt, aby wyliczyć załadunek.")
