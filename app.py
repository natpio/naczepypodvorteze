...
from v_visuals import render_3d, get_pro_color

# Inicjalizacja UI PRO
apply_supreme_ui()
...
with st.sidebar:
    ...
    if st.button(T['add_truss']):
        p2 = {"name": "truss 2m cart", "length": 200, "width": 60, "height": 240, "weight": 420, "canStack": False, "allowRotation": True}
        p3 = {"name": "truss 3m cart", "length": 300, "width": 60, "height": 240, "weight": 630, "canStack": False, "allowRotation": True}
    ...
    try:
        with open('products.json', 'r', encoding='utf-8') as f: inv = json.load(f)
    except: inv = []
    
    sku = st.selectbox(T['sku_sel'], [p['name'] for p in inv], index=None)
    if sku:
        p_ref = next(i for i in inv if i['name'] == sku)
...
with tab_p:
    if st.session_state.v_manifest:
        ...
        for idx, unit in enumerate(assigned):
            ...
            with vc:
                st.plotly_chart(render_3d(veh, unit['s']), use_container_width=True)
                tm, tw = 0, 0
                for s_ in unit['s']:
