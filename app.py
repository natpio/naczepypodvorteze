...
# ==============================================================================
st.set_page_config(page_title="VORTEZA STACK v28.0", layout="wide", initial_sidebar_state="expanded", page_icon="🕋")
...
def apply_enterprise_theme():
    bg = get_base64_img('bg_vorteza.png')
    st.markdown(f"""
        <style>
            ...
            section[data-testid="stSidebar"] {{ background-color: rgba(3, 3, 3, 0.99) !important; border-right: 1px solid rgba(181, 136, 99, 0.3); width: 500px !important; }}
            ...
            /* Load Balancer Track */
            .v-rail {{ width: 100%; height: 30px; background: #050505; border-radius: 15px; position: relative; border: 1px solid #333; margin: 50px 0; }}
            .v-cog {{ position: absolute; width: 12px; height: 60px; top: -15px; background: #00FF41; box-shadow: 0 0 40px #00FF41; border-radius: 6px; transition: left 1.2s cubic-bezier(0.19, 1, 0.22, 1); }}
...
# ==============================================================================
# 4. SILNIK RENDERINGU 3D (EXPLICIT GEOMETRY ENGINE)
# ==============================================================================
...
def build_explicit_cube(x, y, z, dx, dy, dz, color, name):
...
def render_vorteza_3d_pro(veh, stacks):
    ...
    for s in stacks:
        for u in s['items']:
            for p in build_explicit_cube(s['x'], s['y'], u['z'], u['w_fit'], u['l_fit'], u['height'], get_sku_color(u['name']), u['name']): fig.add_trace(p)
...
class V28Engine:
    ...
    def pack(cargo, veh, offset=0):
        # Priorytet: Stacking NO > Rotation NO > Powierzchnia Area
...
def main():
    ...
    with col_l:
        logo = get_base64_img('logo_vorteza.png')
        ...
        if st.session_state.v_manifest:
            ...
            for idx, truck in enumerate(fleet):
                ...
                with v_col:
                    ...
                    st.markdown(f'<div class="v-rail"><div class="v-cog" style="left:{cp}%;"></div><div style="position:absolute;left:10px;top:45px;font-size:0.75rem;color:#666;">{T["axle_f"]}</div><div style="position:absolute;right:10px;top:45px;font-size:0.75rem;color:#666;">{T["axle_r"]}</div></div><br>', unsafe_allow_html=True)
                with d_col:
                    st.markdown(f"**{T['kpi']}**")
